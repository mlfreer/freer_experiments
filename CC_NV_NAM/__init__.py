...
#        print(matrix)
        ...
        num_of_players = len(subsession.get_players())
        cutoff = int(num_of_players/Constants.players_per_group)
#        print(cutoff)

        #resolving ties:
        if investments[cutoff] == investments[cutoff-1]:
            investments[cutoff] = investments[cutoff] - 1
        if investments[2*cutoff] == investments[2*cutoff-1]:
            investments[2*cutoff] = investments[2*cutoff] - 1
        if investments[3*cutoff] == investments[3*cutoff-1]:
            investments[3*cutoff] = investments[3*cutoff] - 1
        ...
        k4=0
        for p in players:
            if (p.invest > investments[cutoff]) and (k1<num_of_groups):
                new_matrix[k1][0] = p.id_in_subsession
                k1=k1+1
                print("1")
            elif (p.invest > investments[2*cutoff]) and (k2<num_of_groups):
                new_matrix[k2][1] = p.id_in_subsession
                k2=k2+1
                print("2")
            elif (p.invest > investments[3*cutoff]) and (k3<num_of_groups):
                new_matrix[k3][2] = p.id_in_subsession
                k3=k3+1
                print("3")
            else:
                new_matrix[k4][3] = p.id_in_subsession
                k4=k4+1
                print("4")
